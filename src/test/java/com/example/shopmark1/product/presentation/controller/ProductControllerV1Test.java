package com.example.shopmark1.product.presentation.controller;

import com.epages.restdocs.apispec.MockMvcRestDocumentationWrapper;
import com.epages.restdocs.apispec.ResourceDocumentation;
import com.epages.restdocs.apispec.ResourceSnippetParameters;
import com.epages.restdocs.apispec.SimpleType;
import com.example.shopmark1.global.presentation.dto.ApiDto;
...
import com.example.shopmark1.user.domain.repository.UserRoleRepository;
...
class ProductControllerV1Test {
    ...
    void testGetProductsWithIdSuccess() throws Exception {
        ...
        mockMvc.perform(
                        get("/v1/products/{id}", productEntity.getId())
                                .header(HttpHeaders.AUTHORIZATION, "Bearer " + accessJwt)
                )
                .andExpectAll(
                        MockMvcResultMatchers.status().isOk(),
                        MockMvcResultMatchers.jsonPath("$.code").value("SUCCESS"),
                        MockMvcResultMatchers.jsonPath("$.data.product.id").value(productEntity.getId().toString())
                )
                .andDo(
    ...
    void testPutProductsWithIdSuccess() throws Exception {
        ...
        ProductEntity updatedEntity = productRepository.findById(UUID.fromString(productId))
                .orElseThrow();
        assertThat(updatedEntity.getName()).isEqualTo(updatedName);
        assertThat(updatedEntity.getPrice()).isEqualTo(15000L);
        assertThat(updatedEntity.getStock()).isEqualTo(70L);
    }
    ...
    void testDeleteProductsWithIdSuccess() throws Exception {
        ...
        ProductEntity deletedEntity = productRepository.findById(UUID.fromString(productId))
                .orElseThrow();
    ...
    private void ensureManagerRole(String username) {
        UserEntity userEntity = userRepository.findByUsername(username)
                .orElseThrow();
        boolean hasManager = userEntity.getUserRoleList()
                .stream()
                .anyMatch(role -> UserRoleEntity.Role.MANAGER.equals(role.getRole()));
        if (!hasManager) {
            UserRoleEntity managerRole = UserRoleEntity.builder()
                    .user(userEntity)
                    .role(UserRoleEntity.Role.MANAGER)
                    .build();
            userRoleRepository.saveAndFlush(managerRole);
            userEntity.add(managerRole);
        }
    }
}
